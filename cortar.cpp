...
void cortar(Img img, Img& img_out){
                ...
                img_out.Largura  = img.Largura/3; // atribui a nova largura da img_out.
                img_out.Altura = img.Altura/3;
                img_out.MaxPixel = img.MaxPixel;

            for(int i = 0; i<img.Altura; i++){
                for(int j = 0; j<img.Largura; j++){

                img_out.Matriz[j][i].R = img.Matriz[j][i].R;
                img_out.Matriz[j][i].G = img.Matriz[j][i].G;
                img_out.Matriz[j][i].B = img.Matriz[j][i].B;
