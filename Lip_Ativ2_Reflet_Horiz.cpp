...
int media = 0;
  ...
  //preenche a primeira metade de de forma inversa.
                img_out.Matriz[i][j].R = img.Matriz[i][img.Altura-j].R;
                img_out.Matriz[i][j].G = img.Matriz[i][img.Altura-j].G;
                img_out.Matriz[i][j].B = img.Matriz[i][img.Altura-j].B;


                }

            }
    //preenche a outra  metade.
            for(int i = 0; i<img.Largura; i++){
                for(int j = img.Altura/2; j<img.Altura; j++){

                img_out.Matriz[i][j].R = img.Matriz[i][j].R;
                img_out.Matriz[i][j].G = img.Matriz[i][j].G;
                img_out.Matriz[i][j].B = img.Matriz[i][j].B;
